...
export default function Home() {
  ...
  const agendaId = useSearchParams().get(`agendaId`);
  ...
  const onSubmit = async (appointment:any) => {
      ...
      if(agendaId){
            ...
            await appointmentService.create(updatedAppointment);
      }
      else{
  ...
  const Body = (
    <div id="body-content" className="flex justify-center items-center h-full w-full flex-grow">
      <form id="appointmentForm" className="grid grid-cols-2 gap-4 h-full items-center m-auto" onSubmit={handleSubmit(onSubmit)}>
        <DataCard title="Cliente" className="col-span-1">
          <input {...fields.client} className="form-control" title="Cliente" placeholder="Nome do Cliente" />
        </DataCard>

        <DataCard title="Serviço" className="col-span-1">
          <input {...fields.service} className="form-control" title="Serviço" placeholder="Nome do Serviço" />
        </DataCard>

        <DataCard title="Dia" cardType="rect" className="col-span-2">
          <input {... fields.date} className="form-control" title="Dia" placeholder="Dia" type="date" />
        </DataCard>

        <DataCard title="Funcionário" className="col-span-1">
          <input {...fields.employee} className="form-control" title="Funcionário" placeholder="Nome do Funcionário" />
        </DataCard>

        <DataCard title="Horário" className="col-span-1">
          <input {...fields.time} className="form-control" title="Hora" type="time" />
        </DataCard>
      </form>
    </div>
  );
